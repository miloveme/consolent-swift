...
- [빠른 시작](#빠른-시작)
- [API 사용법](#api-사용법)
- [OpenAI 호환 API Reference](#openai-호환-api-reference)
- [세션 관리 API](#세션-관리-api)
...
> 채널 서버 모드 세션은 `owned_by: "channel"`로 표시됩니다. 해당 세션으로의 API 요청은 채널 서버 URL로 직접 보내야 합니다.
...
사용 가능한 모델은 `CLIType`에 등록된 CLI 도구에 따라 동적으로 생성됩니다. Agent 모드 세션은 `owned_by: "agent"`, 브릿지 모드 세션은 `owned_by: "bridge"`로 표시됩니다.
...
| 항목 | PTY 모드 (기존) | 채널 서버 모드 |
|------|----------------|---------------|
| API 엔드포인트 | `localhost:9999` (Consolent) | `localhost:8787` (채널 서버) |
| 응답 방식 | 터미널 출력 파싱 | Claude Code 직접 API 응답 |
| Consolent 라우팅 | 정상 라우팅 | 410 Gone + 채널 URL 안내 |
| `GET /v1/models` | `owned_by: "consolent"` | `owned_by: "channel"` |
| 지원 CLI | Claude Code, Gemini, Codex | Claude Code 전용 |
...
- 브릿지 출력 레벨에 따라 원시 CLI 출력도 표시 가능 (debug 레벨)
...
- **API 서버**: 계속 동작 (외부 요청 수신 가능)
- **세션**: 계속 동작 ([헤드리스 터미널](#term-headless-terminal)만 유지)
- **TerminalView**: 비활성화 (렌더링 리소스 절약)
- 윈도우 재표시 시 `outputBuffer` 재주입으로 터미널 화면 복원

### 색상 매핑

메뉴바 메뉴에서 서버와 세션을 색상으로 시각적으로 연결합니다:

- **API Server URL** → 파란색, 해당 세션도 파란색
- **Channel Server URL** → 초록/주황/보라 등 순환 할당, 해당 채널 세션도 동일 색상

### 설정

- **Settings → 일반 → 메뉴바 모드로 시작**: ON 시 앱 시작 시 윈도우를 숨기고 메뉴바 아이콘만 표시

### 세션 순차 생성

동일 CLI 타입(예: Claude Code)의 세션을 여러 개 생성할 때, MCP 서버 등 리소스 충돌을 방지하기 위해 이전 세션의 초기화가 완료된 후 다음 세션을 시작합니다.
