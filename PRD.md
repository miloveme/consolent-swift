...
| CLI 도구 | Adapter | 상태 |
|----------|---------|------|
| Claude Code | `ClaudeCodeAdapter` | 완전 구현 |
| Codex CLI | `CodexAdapter` | 기본 틀 (TUI 패턴 확인 필요) |
| Gemini CLI | `GeminiAdapter` | 기본 틀 (TUI 패턴 확인 필요) |

새 CLI 지원 = `CLIAdapter` 프로토콜 구현 1개 추가.
...
**구현 핵심:**
- PTY 출력 → SwiftTerm 헤드리스 터미널 → 스크린 버퍼 (40행 × 120열)
...
│   ├── Session.swift               # 세션 (PTY + 터미널 + 파서 + Adapter)
...
│       ├── CodexAdapter.swift      # Codex CLI 기본 틀
│       └── GeminiAdapter.swift     # Gemini CLI 기본 틀
├── API/
│   ├── APIServer.swift             # Vapor HTTP/WS 서버 (~800행)
│   └── APIAuthMiddleware.swift     # Bearer token 인증
├── Views/
│   ├── ContentView.swift           # 메인 윈도우 (사이드바 + 터미널 + 상태바)
│   ├── TerminalView.swift          # SwiftTerm NSViewRepresentable 래퍼
...
- [ ] 자동 업데이트 (Sparkle)
- [ ] 에러 핸들링 전반 검토
- [ ] 스트리밍 응답 (`stream: true` in chat/completions)
...
| 동기 응답 | 현재 chat/completions는 blocking. 스트리밍 미구현 |
...
1. **Codex/Gemini TUI 패턴** — 실제 CLI를 실행하여 readySignal, processingSignal, TUI 크롬 패턴을 파악해야 한다. 각 CLI의 버전별 차이도 고려 필요.
2. **스트리밍 응답** — OpenAI API의 `stream: true`를 지원하려면 PTY 출력을 SSE로 중계해야 한다. 응답 완료 시점까지 청크 전송 방식 설계 필요.
3. **세션 복구** — 앱이 크래시되면 실행 중이던 CLI 프로세스는 어떻게 되는가? 프로세스 자체는 살아있을 수 있으니 재연결(reattach) 가능성 검토 필요.
4. **동시 세션 vs 구독 제한** — Claude Code 구독에 동시 사용 제한이 있는 경우, 멀티 세션 사용 시 rate limit에 걸릴 수 있다.
5. **새 CLI Adapter 기여** — 커뮤니티가 새 CLI Adapter를 추가할 수 있는 구조인지, 플러그인 형태가 필요한지 검토.
